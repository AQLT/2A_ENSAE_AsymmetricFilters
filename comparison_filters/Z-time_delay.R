...
saveRDS(data_x13, "comparison_filters/turning_points_x13.RDS")

data_t <- do.call(c, lapply(list_f_t, function(x){
    print(x)
    extract_tp(x)
    }))
saveRDS(data_t, "comparison_filters/turning_points_rkhs_timeliness.RDS")

data_t_rw <- do.call(c, lapply(list_f_t_rw, function(x){
    print(x)
    extract_tp(x)
}))
saveRDS(data_t_rw,"comparison_filters/turning_points_rkhs_timeliness_rw.RDS")

data_lc <- do.call(c, lapply(list_f_lc, function(x){
    print(x)
    extract_tp(x)
}))
saveRDS(data_lc, "comparison_filters/turning_points_lc.RDS")

data_ql <- do.call(c, lapply(list_f_ql, function(x){
    print(x)
    extract_tp(x)
}))
saveRDS(data_ql, "comparison_filters/turning_points_ql.RDS")

data_cq <- do.call(c, lapply(list_f_cq, function(x){
    print(x)
    extract_tp(x)
}))
saveRDS(data_cq, "comparison_filters/turning_points_cq.RDS")

data_daf <- do.call(c, lapply(list_f_daf, function(x){
    print(x)
    extract_tp(x)
}))
saveRDS(data_daf, "comparison_filters/turning_points_daf.RDS")


data_x13 <- readRDS("comparison_filters/turning_points_x13.RDS")
data_t <- readRDS("comparison_filters/turning_points_rkhs_timeliness.RDS")
data_t_rw <- readRDS("comparison_filters/turning_points_rkhs_timeliness_rw.RDS")
data_lc <- readRDS("comparison_filters/turning_points_lc.RDS")
data_ql <- readRDS("comparison_filters/turning_points_ql.RDS")
data_cq <- readRDS("comparison_filters/turning_points_cq.RDS")
data_daf <- readRDS("comparison_filters/turning_points_daf.RDS")
...
saveRDS(data_tot, file = "comparison_filters/full_turning_points.RDS")

...
After a description of the general properties of a linear filter (section \@ref(sec:propMM)), we described the linear filters methods developed by @proietti2008, @ch15HBSA, @trilemmaWMR2019 and @dagumbianconcini2008 (sections \@ref(sec:lppfilters) to \@ref(sec:Dagum)).
...
An \faIcon{r-project} interface is implemented in the package `rjdfilters`^[https://github.com/palatej/rjdfilters.] was developed during this internship.
...
In this paper, we consider the method that consist to extend the minimization problem of local polynomial filters adding the Timeliness criterion defined by @ch15HBSA^[This method is for example coded in Java by Jean Palate in https://github.com/palatej/jdemetra-core.].
...
In @dagumbianconcini2015, the authors suggest performing an optimal bandwidth selection (parameter $b$), decomposing the mean squared revision error as in equation \@ref(eq:msedecomp) but with a uniform spectarl density ($h(\omega)=1$).
...
], the DAF is the worst filters since it introduces a delay of one month until $q=3$. 

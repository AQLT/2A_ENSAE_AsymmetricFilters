...
If the classic asymmetric moving averages have good properties regarding the size of future revisions induces by the process^[See for example @pierce1980SA.], they induce phase shifts that impact the real-time estimation of turning points, introducing time delay in the detection.
...
After a description of the general properties link between seasonal adjustment and trend-cycle estimation (\@ref(sec:SAtoTCE)), we described the linear filters methods developed by @proietti2008, @ch15HBSA, @trilemmaWMR2019 and @dagumbianconcini2008 (sections \@ref(sec:lppfilters) to \@ref(sec:Dagum}).
...
This link also explains that in this paper, all the methods used are implemented in the core libraries of JDemetra+ ^[https://github.com/jdemetra/jdemetra-core.], the seasonal adjustment software recommended by Eurostat. 
An \faIcon{r-project} interface is implemented in the package `rjdfilters`^[https://github.com/palatej/rjdfilters.] was developped during this internship.
...
That's why the filters of the approach of  @trilemmaWMR2019 (section \@ref(sec:WildiMcLeroy)) are described but not used in this report.
...
$$
where $\mu_t$ is the signal (trend) and $\varepsilon_{t}\overset{i.i.d}{\sim}\mathcal{N}(0,\sigma^{2})$ is the noise^[The time series is therefore seasonnally adjusted]. 
...
- the number of neighboured $H=2h+1$ (or the *bandwidth* $h$).
...
When $U=\begin{pmatrix}1&\cdots&1\end{pmatrix}'$, $Z=\begin{pmatrix}-h&\cdots&+h\end{pmatrix}'$, $\delta=\delta_1$, $D=\sigma^2I$ and when the symmetric filter is the Henderson filter we obtain the Musgrave asymmetric filters (used in the seasonal adjustment algorithm X-12ARIMA, see @musgrave1964set). 
...
1. *Linear-Constant* (LC): $y_t$ linear ($d=1$) and $v$ preserve constant signals ($d^*=0$). 
...
## Extension with the revision criteria
...
In this paper, we consider the method that consist to extend the minimization problem of local polynomial filters adding the Timeliness criteria defined by @ch15HBSA^[This method is for example coded in Java by Jean Palate in https://github.com/palatej/jdemetra-core.].
...
In this section we compare the different asymmetric filters applying them to the French industrial production index of manufacture of motor vehicles, trailers and semi-trailers^[This series is produced by INSEE, the French National Statistical Office and is available at https://bdm.insee.fr/series/sdmx/data/SERIES_BDM/010537940.]. 
...
], the DAF is the worst filters since it introduce a delay of one month until $q=3$. 
...
The real-time estimatesmethods that use this filter could be easily improved.
...
<!-- Regarding areas for improvement / opening, I noted: -->
<!-- - issues related to high frequency series / irregular series. In addition to the problems of choosing the length of the filter, we should see if we find the same results as for short filters. -->
<!-- - study the properties of asymmetric filters induced by model-based approaches (like what Wildi did for TRAMO-SEATS). -->
<!-- - study the impact of atypical points: in X-11 there is a strong correction of atypical points on the irregular before applying the filters to extract the trend (I had also read a few weeks ago that 'about 50% of the points were corrected by this automatic procedure: does this statistic ring a bell?). This therefore prompts us to study what would be their impact on the estimation of trends / turning points and to study robust methods. -->


<!-- Concernant les pistes d’amélioration/ouverture, j’ai noté : -->
<!-- - les enjeux (issue/concern) liées aux séries haute fréquence/séries irrégulières. Outre les problèmes de choix de la longueur du filtre, il faudrait voir si on retrouve les mêmes résultats que pour des filtres courts. -->
<!-- - étudier les propriétés des filtres asymétriques induits par les approches model-based (comme ce que Wildi a fait pour TRAMO-SEATS). -->
<!-- - étudier l’impact des points atypiques : dans X-11 il y a une forte correction des points atypiques sur l’irrégulier avant d’appliquer les filtres pour extraire la tendance (j’avais d’ailleurs lu il y a quelques semaines qu’environ 50 % des points étaient corrigés par cette procédure automatique : cette statistique vous dit quelque chose ?). Cela pousse donc à étudier quel serait leur impact sur l’estimation des tendances/points de retournement et à étudier des méthodes robustes. -->

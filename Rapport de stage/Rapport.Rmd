...
$$
The function $\Gamma_\theta(\e^{i \omega k})=\sum_{k=-p}^{+f} \theta_k e^{i \omega k}$ is called the *transfer function* or *frequency response function*.
...
1. *Linear-Constant* (LC): $y_t$ linear ($d=1$) and $v$ preserve constant signals ($d^*=0$), $v$ depend on the ratio $\delta_1^2/\sigma^2$. We obtain Musgrave filters when the Henderson kernel is used.

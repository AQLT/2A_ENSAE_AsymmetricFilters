...
Business cycle analysis, and in particular the early detection of turning points in a series, is a major topic in the analysis of economic outlook.
...
For example, the X-12-ARIMA seasonal adjustment method uses Henderson moving averages and composite moving averages to estimate the main components of a time series, while TRAMO-SEATS uses Wiener-Kolmogorov filters. 
...
For example, X-12-ARIMA or TRAMO-SEATS apply symmetrical averages to the forecasts obtained from an ARIMA model of the series. However, since the predicted values are linear combinations of past values, it consist in applying to use methods do use skewed moving averages at the end of the series.
...
Due to the link between seasonal adjustment and trend-cycle extraction (section \@ref(sec:SAtoTCE)), we here focus on non-parametric methods that could be included in X-12-ARIMA.
After a description of the general properties link between seasonal adjustment and trend-cycle estimation (\@ref(sec:SAtoTCE)), we described the linear filters methods developed by @proietti2008, @ch15HBSA, @ch16HBSA and @dagumbianconcini2008 (sections \@ref(sec:lppfilters) to \@ref(sec:Dagum}).
...
Thus, the estimation of trend-cycle component is linked to the seasonal component.
...
In this paper we focus on non-parametric methods
...
When $U=\begin{pmatrix}1&\cdots&1\end{pmatrix}'$, $Z=\begin{pmatrix}-h&\cdots&+h\end{pmatrix}'$, $\delta=\delta_1$, $D=\sigma^2I$ and when the symmetric filter is the Henderson filter we obtain the Musgrave asymmetric filters (used in the seasonal adjustment algorithm X-12-ARIMA, see @musgrave1964set). 
...
This approach can be called as the "FST approach" in reference in reference to the three indicators used in the optimization problem.
...
The figures \@ref(fig:lppguglc) shows the impact of $\alpha_T$ on the coefficients of the linear filter with the LC method:
...
<!-- \caption{Revision error of the different real-time ($q=0$) asymmetric filters for $h=6$, for the IPI-FR.}\label{fig:cl2all}\footnotesize -->
<!-- \emph{Note: to see the animation, the PDF must be open with Acrobat Reader, KDE Okular, PDF-XChange or Foxit Reader. -->
<!-- Otherwise you will only be able to see the results for the Henderson kernel.} -->
<!-- \end{figure} -->
...
<!-- \colorbox{BurntOrange}{Appliquer à une donnée réelle} -->

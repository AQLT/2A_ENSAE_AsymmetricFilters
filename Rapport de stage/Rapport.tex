...
In this paper we focus on the one defined by \textcite{ch15HBSA} and \textcite{trilemmaWMR2019} to build the asymmetric filters.
...
The \textbf{Henderson filter} is a specific case of a local cubif fit (\(p=3\)), widely used for trend estimation (for example it's the filter used in the seasonal adjustment software X-12-ARIMA). For a fixed bandwith, Henderson found the kernel that gave the smoothest estimates of the trend.
...
\item
  ...
  \]
\item
  \emph{Smoothness}, \(S_g\): it's the Henderson smoothness criteria (sum of the squared of the third difference of the coefficients of the filter). It measures the flexibility of the coefficient curve of a filter and the smoothness of the trend.
  \[
  S_g = \sum_{j}(\nabla^{3}\theta_{j})^{2}
  \]
\item
  \emph{Timeliness}, \(T_g\): it mesures the phase-shift between input and output signal for specific frequencies and depends on the gain and phase shift functions (\(\rho_\theta\) and \(\varphi_{\theta}\)). In this paper we focus on the frequencies less than \(2\pi/12\) (cycle of one year).
  \[
  \int_{0}^{2\pi/12}\rho_{\theta}(\omega)\left|\varphi_{\theta}(\omega)\right|^{2}\ud\omega
  \]
\end{itemize}

\hypertarget{data-dependent-filter-trilemmawmr2019-secwildimcleroy-secguguemosetal}{%
\section{\texorpdfstring{Data-dependent filter: \textcite{trilemmaWMR2019} \{\#sec:WildiMcLeroy\} sec:GuguemosEtAl}{Data-dependent filter: @trilemmaWMR2019 \{\#sec:WildiMcLeroy\} sec:GuguemosEtAl}}\label{data-dependent-filter-trilemmawmr2019-secwildimcleroy-secguguemosetal}}

...
Ce rapport décrit et compare différentes approches pour construire des filtres asymétriques: l'ajustement de polynôme locaux, des méthodes basées sur une optimisation des propriétés des filtres (l'approche \emph{Fidelity-Smoothness-Timeliness} --- Fidélité-Lissage-Temporalité ---, FST, approche et un filtre dépendant des données) et des filtres basés sur les espaces de Hilbert à noyau reproduisant (RKHS).
...
An \faIcon{r-project} interface is implemented in the package \texttt{rjdfilters}\footnote{Available at \url{https://github.com/palatej/rjdfilters}.} and was developed during this internship.
